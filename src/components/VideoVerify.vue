<template>
  ...
  >
    ...
    />
    <!--  成功 -->
    <div class="face-tips" v-if="step === 2">
      <div class="title">多动作活体验证</div>
...
<script>
  export default {
    ...
    methods: {
      ...
      getActionData() {
        this.actionVal = [1, 5]
        this.$uMessageBox.show({
          title: '动作提示',
          submitText: '录制',
          closeOnClickOverlay: false,
          message: `<div class="action-p">先${this.actionList[this.actionVal[0]].text}、后${this.actionList[this.actionVal[1]].text}</div><ul class="action-list"><li>
    <img src="${this.actionList[this.actionVal[0]].icon}" alt=""></li><li>
    <img src="${this.actionList[this.actionVal[1]].icon}" alt=""></li></ul>`,
          onConfirm: () => {
      ...
      changeVideo() {
        ...
        if (files && files.length > 0) {
          ...
          if (file.size > 10 * 1024 * 1024) {
            that.$toast('视频大小超限，超出 10 MB,请重新录制');
            return;
          }
          that.$toast.loading({
            message: '上传中...',
